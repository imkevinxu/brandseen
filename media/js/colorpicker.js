    ...
    ColorPicker = function(x, y, size, initcolor, element, selectors, dimensions, separation, isColorblind) {
      ...
      if (Modernizr.touch) {
        t.disc.drag((function(dx, dy, x, y) {
          return t.docOnMove(dx, dy, x, y);
        }), (function(x, y) {
          t.hsOnTheMove = true;
          return t.setHS(x - t.x, y - t.y);
        }), function() {
          return t.hsOnTheMove = false;
        });
      } else {
        wheelEl = document.getElementById(element);
        if (element !== "wheel") {
          containerEl = document.getElementById("popcontainer");
        }
        if (element === "wheel") {
        ...
        window.onmousemove = function(e) {
          if (offset.top) {
            return t.setHS(e.clientX - offset.left, e.clientY - offset.top);
          } else {
